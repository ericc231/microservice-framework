...
# LDAP Auth REST Service Start Script
...
JAVA_OPTS="$JAVA_OPTS -Dspring.ldap.urls=${LDAP_URL:-ldap://localhost:8389}"
JAVA_OPTS="$JAVA_OPTS -Dspring.ldap.base=${LDAP_BASE:-dc=springframework,dc=org}"
...
# Start the service
echo "Starting LDAP Auth REST Service..."
echo "Port: $SERVICE_PORT"
echo "LDAP Server: ${LDAP_URL:-ldap://localhost:8389}"
echo "LDAP Base DN: ${LDAP_BASE:-dc=springframework,dc=org}"
...
if ps -p $PID > /dev/null 2>&1; then
    ...
    for i in {1..10}; do
        ...
        if [ $i -eq 10 ]; then
            ...
            echo "Note: LDAP service requires LDAP server to be running"
        fi
        sleep 2
    done
else
